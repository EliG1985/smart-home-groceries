type InvitePlatform = 'android' | 'ios';
...
const getInviteBaseUrl = (): string =>
  trimTrailingSlash(getEnv('INVITE_PUBLIC_BASE_URL') || DEFAULT_INVITE_BASE_URL);

const getAppScheme = (): string => getEnv('APP_SCHEME') || DEFAULT_APP_SCHEME;

const getAndroidPackageName = (): string =>
  getEnv('ANDROID_APP_PACKAGE') || DEFAULT_ANDROID_PACKAGE;

const getAndroidStoreUrl = (): string =>
  getEnv('ANDROID_STORE_URL')
  || `https://play.google.com/store/apps/details?id=${encodeURIComponent(getAndroidPackageName())}`;

const getIosStoreUrl = (): string =>
  getEnv('IOS_STORE_URL') || 'https://apps.apple.com/app/id0000000000';

export const buildInviteUrls = (token: string) => {
...
export const getInviteUniversalLinkHost = (): string => {
  try {
...
export const getAndroidAppPackage = (): string => getAndroidPackageName();

export const getAndroidShaFingerprints = (): string[] =>
  (getEnv('ANDROID_APP_SHA256_CERT_FINGERPRINTS') || '')
    .split(',')
    .map((value) => value.trim())
    .filter(Boolean);

export const getIosAssociatedAppIds = (): string[] =>
  (getEnv('IOS_ASSOCIATED_APP_IDS') || '')
    .split(',')
    .map((value) => value.trim())
    .filter(Boolean);

const getStoreUrlForPlatform = (platform: InvitePlatform): string =>
  platform === 'ios' ? getIosStoreUrl() : getAndroidStoreUrl();

export const renderInviteLandingPage = (token: string, inviteStatus: string) => {
  const { appInviteUrl, publicInviteUrl, androidStoreUrl, iosStoreUrl } = buildInviteUrls(token);
